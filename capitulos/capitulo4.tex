...
Adicionalmente, foram introduzidos campos na \textbf{tabela de reciclagens} da base de dados para garantir rastreabilidade e contexto:
\begin{itemize}
    \item \texttt{user\_id}: Identificador do utilizador autenticado que submeteu o código de barras/registo da separação.
    \item \texttt{localizacao}: Indicação da localização onde a separação foi realizada, com valores “\textit{dentro\_ipbeja}” ou “\textit{fora\_ipbeja}”.
\end{itemize}
Deste modo, sempre que um utilizador insere um código de barras, o registo passa a incluir o \textbf{id do utilizador} e a \textbf{localização} (fora ou dentro do IPBeja), permitindo auditoria e análises mais precisas.
